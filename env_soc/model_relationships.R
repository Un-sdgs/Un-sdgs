...
rel <- read_csv("env_soc_synergies_sub_dataOver20.csv")
...
for(i in 1:dim(rel)[1]){
  ...
  soc <- read_csv(paste("clean_",rel$soc_sub_code[i],".csv",sep=""))
  env <- env[,1:4]
  ...
  fit <- lm(log(soc_value+.5) ~ log(env_value+.5)+log(GDP)+log(Population)+region,data=combn)
  x <- summary(fit)
...
write_csv(rel,"soc_env_synergies_results.csv")
